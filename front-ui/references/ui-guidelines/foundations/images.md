...
When the author has not supplied `alt`, the skill drafts it with a **local vision model running on Ollama** (`gemma3:4b`). Local-only — nothing leaves the machine. Runtime is Python 3.9+.
