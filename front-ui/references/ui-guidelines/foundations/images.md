...
When the author has not supplied `alt`, the skill drafts it with a **local vision model running on Ollama** (default `gemma4:e4b`; `gemma4:e4b-mlx` on MLX-capable hardware). Local-only — nothing leaves the machine. Runtime is Python 3.9+.
