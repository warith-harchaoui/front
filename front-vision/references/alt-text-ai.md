...
- **MLX-capable hardware variant:** `gemma4:e4b-mlx` (selected automatically)
...
| `Error: model not found` | Tag mistyped or installer skipped | Re-run installer, or `ollama pull gemma4:e4b` directly. |
...
The same Ollama setup powers `python front-publish/scripts/meta_from_ollama.py`, which drafts the page-level `<title>`, `<meta name="description">`, Open Graph, Twitter, and JSON-LD `@type` from a goal description or an HTML page. See `references/meta-tags.md`.
