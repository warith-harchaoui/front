...
The favicon set, theme-color, and manifest are produced by `front/scripts/favicons.py` (see `ui-guidelines/foundations/app-icons.md`).
...
A local AI helper for the description and `og:image:alt` is `front/scripts/meta_from_ollama.py` (see `references/alt-text-ai.md` for the install path; same Ollama setup).
